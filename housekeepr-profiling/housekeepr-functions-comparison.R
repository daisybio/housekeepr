...
calculateTopTable <- function(GSE_samples_annotations, GSE, gset, normfinder = FALSE) {
  ...
  # prepare data for differential analysis
    if (normfinder) {
      norm_table <- as.data.table(cbind(fData(gset), exprs(gset)))
      return(norm_table)
    }
...
calculateBootstrapMatrix <- function(ranking_matrix, bootstrap_replications, bootstrap_sample_size, num_hkg, print_paralog_groups = NULL) {
  ...
  if (!is.null(print_paralog_groups))
    print(bootstrap_matrix[paralog_group %in% print_paralog_groups])
...
Normfinder=function(dat0,Groups=TRUE,ctVal=TRUE,pStabLim=0.25){
  ...
  MakeStab=function(da){
    ...
    for (group in 1:ngr){
      genegroupavg[,group]=apply(as.matrix(da[,grId==groupnames[group]]),1,mean)}
    # Group averages
    groupavg=rep(0,ngr)
    for (group in 1:ngr){groupavg[group]=mean(as.matrix(da[,grId==groupnames[group]]))}
